...
static DWORD WINAPI preCheckThread(LPVOID param) {
    for (int i = 0; i < g_counter; i++) {
        ...
        int tries = 0;
        do {
            h = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, g_processes[i].processID);
            tries++;
        } while (h == INVALID_HANDLE_VALUE && GetLastError() == ERROR_BAD_LENGTH && tries < 3);
        ...
        if (h == INVALID_HANDLE_VALUE) {
            ...
            me.dwSize = sizeof(MODULEENTRY32);
            int count = 0;
            for (BOOL ok = Module32First(h, &me); ok; ok = Module32Next(h, &me))
                count++;
